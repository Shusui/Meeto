			...
			<div id="meet-info">
				<div id = "meet-info-aux">		
					...
					<table class ="tftable">
						<thead>
							<tr>
								<th>Key Decision description</th>
							</tr>
						</thead>
						<c:forEach var="key" items="${session.keys}">
							<tr>
								<td>
									<c:out value = "${key.getDesc()}"/>
								</td>
							</tr>
						</c:forEach>
					</table>
					...
					<table class ="tftable">
						<thead>
							<tr>
								<th>Personal TODO-List</th>
							</tr>
						</thead>
						<c:forEach var="action" items="${session.actions}">
							<tr>
								<td>
									<c:out value = "${action.getDesc()}"/>
								</td>	
							</tr>
						</c:forEach>
					</table>
					...
					<table class ="tftable">
						...
						<tbody>
							<c:forEach var="meet" items="${session.meetings}">
								<tr>
									<td>
										<c:out value = "${meet.getTitle()}"/>
									</td>
									<td>
										<c:out value = "${meet.getLeader()}"/>
									</td>
									<td>
										<c:out value = "${meet.getOutcome()}"/>
									</td>
									<td>
										<c:out value = "${meet.getDate()}"/>
									</td>
									<td>
										<c:out value = "${meet.getLocal()}"/>
									</td>
								</tr>
							</c:forEach>
						</tbody>
			...
			<div id="buttons">
				...
				<div id = "aux_buttons">
					<s:form method="post">
						...
						<s:submit value="Reload" onclick="form.action='reload';"/>
					</s:form>
					<br>
					<s:form method="post">
					...
					<br>
					<s:form method="post">
						<s:textfield name="title" placeholder = "New Title" size = "10"/>
						<s:textfield name="description" placeholder = "New Outcome" size = "10"/>
						<s:textfield name="date" placeholder = "New Date" size = "10"/>
						<s:textfield name="location" placeholder = "New Location" size = "10"/>
						<s:submit value = "Update Meeting" onclick="form.action='update_meeting';"/>
					</s:form>
					<br>
					<s:form method="post">
						<s:textfield name="description" placeholder = "Change Description" size = "10"/>
						<s:textfield name="title" placeholder = "Change Title" size ="10"/>
